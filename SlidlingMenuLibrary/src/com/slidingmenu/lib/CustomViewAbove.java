...
public class CustomViewAbove extends ViewGroup {
	 ...
	 * 
	 * @param position Position index of the first page currently being
	 *        displayed. Page position+1 will be visible if positionOffset is
	 *        nonzero.
	...
	private boolean thisTouchAllowed(MotionEvent ev) {
		...
		if (isMenuOpen()) return mViewBehind.menuOpenTouchAllowed(mContent, mCurItem, x);
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (DEBUG) if (action == MotionEvent.ACTION_DOWN) {
		...
		if (v instanceof ViewGroup) {
			...
			for (int i = count - 1; i >= 0; i--) {
				...
				if (x + scrollX >= child.getLeft() && x + scrollX < child.getRight() && y + scrollY >= child.getTop()
					...
					&& canScroll(child, true, dx, x + scrollX - child.getLeft(), y + scrollY - child.getTop())) return true;
			}
		}
