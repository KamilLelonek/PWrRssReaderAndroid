...
public class PropertyValuesHolder implements Cloneable {
	...
	private static Class[] FLOAT_VARIANTS = { float.class, Float.class, double.class, int.class, Double.class,
		Integer.class };
	private static Class[] INTEGER_VARIANTS = { int.class, Integer.class, float.class, double.class, Float.class,
		Double.class };
	private static Class[] DOUBLE_VARIANTS = { double.class, Double.class, float.class, int.class, Float.class,
		Integer.class };
	 ...
	 *        frame to provide the necessary interpolation between the Object
	 ...
	 *        values. This can be the actual property name to be used when using
	 *        a ObjectAnimator object, or just a name used to get animated
	...
	public static PropertyValuesHolder ofKeyframe(String propertyName, Keyframe... values) {
		...
		if (keyframeSet instanceof IntKeyframeSet) return new IntPropertyValuesHolder(propertyName,
			(IntKeyframeSet) keyframeSet);
		else if (keyframeSet instanceof FloatKeyframeSet) return new FloatPropertyValuesHolder(propertyName,
	...
	public static PropertyValuesHolder ofKeyframe(Property property, Keyframe... values) {
		KeyframeSet keyframeSet = KeyframeSet.ofKeyframe(values);
		if (keyframeSet instanceof IntKeyframeSet) return new IntPropertyValuesHolder(property,
			(IntKeyframeSet) keyframeSet);
		else if (keyframeSet instanceof FloatKeyframeSet) return new FloatPropertyValuesHolder(property,
			(FloatKeyframeSet) keyframeSet);
		else {
			PropertyValuesHolder pvh = new PropertyValuesHolder(property);
			pvh.mKeyframeSet = keyframeSet;
	 ...
	 *        PropertyValuesHolder. This type is used as a first guess at the
	 *        parameter type, but we check for methods with several different
	 *        types to avoid problems with slight mis-matches between supplied
	 *        values and actual value types used on the setter.
		...
		if (valueType == null) {
			try {
				...
				try {
					...
					Log.e("PropertyValuesHolder", "Couldn't find no-arg method for property " + mPropertyName + ": "
						+ e);
				}
			}
		}
	...
	void setupSetterAndGetter(Object target) {
		if (mProperty != null) {
			...
			try {
				...
				Log.e("PropertyValuesHolder", "No such property (" + mProperty.getName() + ") on target object "
					+ target + ". Trying reflection instead");
				mProperty = null;
			}
		}
	...
	void init() {
		if (mEvaluator == null) {
			...
			// equivalents
			mEvaluator = (mValueType == Integer.class) ? sIntEvaluator : (mValueType == Float.class) ? sFloatEvaluator
				: null;
		}
		if (mEvaluator != null) {
	 ...
	 *        the method name after the prefix. The first letter of this word
	...
	static String getMethodName(String prefix, String propertyName) {
		if (propertyName == null || propertyName.length() == 0) // shouldn't get here
		return prefix;
