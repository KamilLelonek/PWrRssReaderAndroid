...
public abstract class Watson extends FragmentActivity implements OnCreatePanelMenuListener, OnPreparePanelListener,
			...
			if (mFragments.mAdded != null) {
				for (int i = 0; i < mFragments.mAdded.size(); i++) {
					...
					if (f != null && !f.mHidden && f.mHasMenu && f.mMenuVisible
						...
						if (((OnOptionsItemSelectedListener) f).onOptionsItemSelected(item)) return true;
					}
				}
			}
		}
