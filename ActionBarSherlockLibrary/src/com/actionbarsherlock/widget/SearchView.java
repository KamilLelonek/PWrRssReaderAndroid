...
public class SearchView extends LinearLayout implements CollapsibleActionView {
	...
	*/
	...
	public SearchView(Context context, AttributeSet attrs) {
		...
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.FROYO) throw new IllegalStateException(
			"SearchView is API 8+ only.");
	 ...
	 *        performs actions in the SearchView such as clicking on buttons or
	 *        typing a query.
	 */
	...
	public CharSequence getQueryHint() {
		if (mQueryHint != null) return mQueryHint;
	 ...
	 *        false if only those items that have a query refinement flag set
	...
	View.OnKeyListener mTextKeyListener = new View.OnKeyListener() {
		...
		public boolean onKey(View v, int keyCode, KeyEvent event) {
			...
			if (mQueryTextView.isPopupShowing() && mQueryTextView.getListSelection() != AdapterView.INVALID_POSITION) return onSuggestionsKey(
				v, keyCode, event);
	...
	private boolean onSuggestionsKey(View v, int keyCode, KeyEvent event) {
		...
		if (event.getAction() == KeyEvent.ACTION_DOWN && KeyEventCompat.hasNoModifiers(event)) {
			...
			if (keyCode == KeyEvent.KEYCODE_DPAD_UP && 0 == mQueryTextView.getListSelection()) // TODO: restoreUserQuery();
