...
public class ActionBarSherlockCompat extends ActionBarSherlock implements MenuBuilder.Callback,
	com.actionbarsherlock.view.Window.Callback, MenuPresenter.Callback, android.view.MenuItem.OnMenuItemClickListener {
	...
	public boolean dispatchOpenOptionsMenu() {
		...
		if (!isReservingOverflow()) return false;
	...
	public boolean dispatchCloseOptionsMenu() {
		...
		if (wActionBar != null) return wActionBar.hideOverflowMenu();
		return false;
	}
	...
	private boolean preparePanel() {
		...
		if (mMenuIsPrepared) return true;
		
		// Init the panel state's menu--return false if init failed
		if (mMenu == null || mMenuRefreshContent) {
			if (mMenu == null) {
				if (!initializePanelMenu() || (mMenu == null)) return false;
			}
	...
	private void updateInt(int featureId, int value, boolean fromResume) {
		...
		if ((getFeatures() & featureMask) == 0 && !fromResume) return;
	...
	private IcsProgressBar getCircularProgressBar(boolean shouldInstallDecor) {
		if (mCircularProgressBar != null) return mCircularProgressBar;
	...
	private IcsProgressBar getHorizontalProgressBar(boolean shouldInstallDecor) {
		if (mHorizontalProgressBar != null) return mHorizontalProgressBar;
		if (mContentParent == null && shouldInstallDecor) {
			installDecor();
		}
	...
	public boolean requestFeature(int featureId) {
		...
		if (mContentParent != null) throw new AndroidRuntimeException(
	...
	private ViewGroup generateLayout() {
		...
		if (!a.hasValue(R.styleable.SherlockTheme_windowActionBar)) throw new IllegalStateException(
		...
		if (contentParent == null) throw new RuntimeException("Couldn't find content container view");
	...
	public static String cleanActivityName(String manifestPackage, String activityName) {
		if (activityName.charAt(0) == '.') //Relative activity name (e.g., android:name=".ui.SomeClass")
		return manifestPackage + activityName;
		if (activityName.indexOf('.', 1) == -1) //Unqualified activity name (e.g., android:name="SomeClass")
		return manifestPackage + "." + activityName;
