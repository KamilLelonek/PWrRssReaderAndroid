...
public final class ResourcesCompat {
	...
	public static boolean getResources_getBoolean(Context context, int id) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB_MR2) return context.getResources().getBoolean(id);
		...
		if (id == R.bool.abs__action_bar_embed_tabs) {
			if (widthDp >= 480) return true; //values-w480dp
			return false; //values
		}
		if (id == R.bool.abs__split_action_bar_is_narrow) {
			if (widthDp >= 480) return false; //values-w480dp
			return true; //values
		}
		if (id == R.bool.abs__action_bar_expanded_action_views_exclusive) {
			if (smallestWidthDp >= 600) return false; //values-sw600dp
			return true; //values
		}
	...
	public static int getResources_getInteger(Context context, int id) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB_MR2) return context.getResources().getInteger(id);
		
		DisplayMetrics metrics = context.getResources().getDisplayMetrics();
		float widthDp = metrics.widthPixels / metrics.density;
		
		if (id == R.integer.abs__max_action_buttons) {
			if (widthDp >= 600) return 5; //values-w600dp
			if (widthDp >= 500) return 4; //values-w500dp
			if (widthDp >= 360) return 3; //values-w360dp
			return 2; //values
		}
